...
type Provider = {
  ...
  fixedCompat?: Compat; // 固定兼容类型，绕过自动检测
};
...
const getStatsText = (): string => {
  ...
  const lines = [
    '📊 <b>AI 使用统计</b>', '',
    `总请求: ${usageStats.totalRequests} 成功率: ${rate}% 平均延迟: ${usageStats.averageLatency.toFixed(0)}ms`, '',
    '<b>按提供商:</b>'
  ];
  for (const [p, s] of Object.entries(usageStats.providerStats)) {
    const r = ((s.requests - s.errors) / Math.max(1, s.requests) * 100).toFixed(1);
    lines.push(`  • ${p}: ${s.requests}次 (成功率${r}%, ${s.avgLatency.toFixed(0)}ms)`);
  }
  return lines.join('\n');
};
...
/* ---------- 帮助文案 ---------- */
const help_text = `🔧 ✨ <b>新增功能</b>
• 💾 智能缓存：相同问题3分钟内快速回复
• 📊 使用统计：.ai stats 查看 API 调用情况
• 🚀 性能优化：更智能的图片压缩
      ...
      try {
        ...
        if (subn === "stats" || subn === "stat") {
          ...
          if (sub === "reset" || sub === "clear") {
            resetStats();
            await msg.edit({ text: "✅ 统计已重置", parseMode: "html" });
          } else {
